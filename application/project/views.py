...
class ProjectViewSets(mixins.ListModelMixin, mixins.CreateModelMixin,
    ...
    def create(self, request, *args, **kwargs):
        ...
        if _project.exists():
            return JsonResponse(code=10080, data='project name already exist')
        ...
        if cname:
            ...
            if not copied_project_queryset.exists():
                return JsonResponse(code=10088, data='copy project not exist')
        ...
        groups_queryset = UserGroup.objects.filter(group__user_set__in=[current_user])
